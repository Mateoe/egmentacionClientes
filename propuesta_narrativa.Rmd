...
crear_boxplot <- function(datos, variable, etiqueta, extremos = TRUE, ceros=TRUE){
    ...
    if (extremos == FALSE) {
        ...
        Q90 <- datos %>% 
               select(!!variable) %>% summarise(percentil = quantile(!!variable ,0.95))
...
<div style='text-align: justify;'>
2. Las datos categóricos cuentan con una tabla adicional que registra la proporción de empresas que se encuentran en cada nivel.
</div>
...
3. Similar a los resultados del canal anterior, el grupo 2 presenta valores monetarios mas altos donde la media de este es significativamente mayor, lo cual se evidencia además en el resto de medidas.
...
crear_boxplot(datos_segmentados, en_vm_canal3, extremos = FALSE, ceros = FALSE,
              etiqueta = "ticket promedio de entrada por el canal 3")
```
...
```{r, fig.height = 6, fig.width = 9, message=FALSE}
crear_boxplot(datos_segmentados, en_vm_canal10, extremos = FALSE, ceros = FALSE,
...
crear_boxplot(datos_segmentados, en_tx_canal3, extremos = FALSE, ceros = FALSE,
              etiqueta = "transacciones de entrada por el canal 3")
```
...
crear_boxplot(datos_segmentados, en_tx_canal4, extremos = FALSE, ceros = FALSE,
              etiqueta = "transacciones de entrada por el canal 4")
```
...
<div style='text-align: justify;'>
En este canal el grupo 1 se muestra altamente inactivo, donde más del 75% de los clientes no han realizado ninguna transacción, mostrando una clara inactividad en comparación con los integrantes del grupo 2, los cuales en su mayoría han realizado más de 3.40 transacciones.
</div>
...
<div style='text-align: justify;'>
El canal 7 muestra una alta inactividad por parte de ambos grupos, sin embargo, el grupo 2 es claramente más activo al presentar una menor cantidad de datos en cero y un valor máximo mucho mas alto.
</div>
...
```{r, fig.height = 6, fig.width = 9, message=FALSE}
crear_boxplot(datos_segmentados, en_tx_canal7, extremos = FALSE, ceros = FALSE,
...
<div style='text-align: justify;'>
En el canal 8 se evidencia que el grupo 1 es claramente más inactivo con un número máximo de transacciones inferior a las 39 y una proporción de ceros superior al 75%. Por su parte el grupo 2 continúa mostrando más actividad y numero mayores en cada una de las medidas. 
</div>
...
El grupo 2 muestra una disminución en la proporción de ceros, presentando una inactividad menor y un número de transacciones realizadas levemente más alto.
</div>
...
El grupo 2 presenta una proporción de entradas en cero significativamente más baja, reflejando una mayor actividad y presentando valores más altos en cada una de las mediadas de resumen, lo cual indica que está constituido por clientes económicamente más fuertes.
</div>
...
```{r, fig.height = 6, fig.width = 9, message=FALSE}
crear_boxplot(datos_segmentados, sal_tx_otros, extremos = FALSE, ceros = FALSE,
...
<div style='text-align: justify;'>
La tabla de proporciones nos permite observar de una mejor manera lo mencionado anteriormente dado que para el los dos primeros niveles se observa una clara superioridad del grupo 1, indicando que se constituye en gran medida por clientes que tienen una baja o moderadamente baja cantidad de cuentas por pagar, comportamiento que se invierte a partir del nivel 3, donde el grupo 2 empieza a dominar, indicando que posee una proporción mayor de empresas con gran cantidad de cuentas por pagar.
</div>
...
<div style='text-align: justify;'>
El resultado de la tabla de proporciones solo confirma dicho previamente, el grupo 2 continúa manteniendo valores mas altos que los del grupo 1.
</div>
